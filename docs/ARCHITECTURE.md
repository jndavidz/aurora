...
> 更新时间: 2026-08-13
> 关联: `docs/DEEPSEEK.md`(DeepSeek 接入)、`docs/NAS_DEPLOYMENT.md`(部署)。
...
| `internal/config/config.go` | DeepSeek 环境变量(`DEEPSEEK_WEB_*`、`DEEPSEEK_MODELS`) |
| `internal/toolcall/` | 标签参数化(`TagSet`,ChatGPT / DeepSeek 双标签) |
...
联网搜索受账号/套餐限制,DeepSeek 快速模式识图与联网搜索互斥,均如实标注,避免客户端误用。
