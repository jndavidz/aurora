...
| **DeepSeek** | userToken(localStorage) | `deepseek_tokens.txt` | 会话级(实测稳定) | **不在 harvester 范围**(2026-09-05 二次确认:NUC Chrome 的 deepseek 为**游客会话**——页面可免登录对话,userToken 92B 单段非 JWT,上游 invalid token,刷新/cookie 均无认证项;有效票 64B 人工管理)。**重新启用条件**:NUC Chrome 真实登录 deepseek 账号(userToken 变多段 JWT)后加回 harvester 并 live 验证 |
