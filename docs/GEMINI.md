...
4. **令牌轮换规律(2026-08-14 实测,推翻"会话级固定复用"旧结论)**:
   整套令牌(`at`/`SNlM6e`/`f.sid`/会话 UUID)与**页面实例**绑定,页面实例切换
   (新会话首条消息、刷新、浏览器重启、崩溃恢复)即全部轮换;在**已有会话里
   续发消息不轮换**(URL 不变)。桥的缓存令牌只在"页面静止"期间有效。
   失效信号:`wrb.fr` 错误帧 `[...,[13]]` 或 BardErrorInfo 1060/1096/1157。
   自愈:在浏览器当前会话里**再发一条消息**(若刚发的是新会话首条,再发第二条),
   桥的 Network 监听自动捕获新实例令牌即恢复 —— 无需跑脚本、无需 reload
   (reload 会再次切换实例,破坏恢复)。
