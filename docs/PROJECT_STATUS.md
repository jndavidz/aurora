...
| **G3 合并完成** | `internal/sentinelvm` 共享底层上线(09-05):寄存器底座 Base(SetReg/GetReg/CopyQueue/CallFn/DerefArgs,RegKey 统一 NaN 安全版)+ Latin-1/base64/XOR 纯函数,so/turnstile 嵌入接入,vmFunc 类型统一——**-190 行重复**。语义差异层(asNumber 失败值 0 vs NaN、jsToString NaN/Infinity、jsJSONStringify 裸 Marshal vs JS 语义、buildWindow 环境)**经逐函数 diff 证实保留各自**——合并边界以 golden 全绿划定,合并即改行为的部分不做。so 1107→1009 / turnstile 1641→1545 / +136 |
