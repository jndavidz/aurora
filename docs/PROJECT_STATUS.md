...
| G1 终态:NUC 统一凭证提取器 | 架构事实（用户拍板 09-05）:**各模型登录态权威在 NUC Chrome**。`scripts/nuc/token-harvester.{mjs,service,timer}`（已部署 NUC 并注册 timer 每日 07:00+rand30min）：CDP 读各站 localStorage/cookie → md5 幂等推 NAS 部署区 → E3 热加载生效。站点 minimax/mimo/qianwen/grok（qianwen 因新鲜 x5sec **通道复活**;deepseek 二次确认为游客会话,保持人工);排除豆包(冻结)/GLM·Kimi(自愈)/DeepSeek(游客票无效,人工)/桥体系。教训链一并入库:首批 NAS 侧脚本方向错误（Drive 排除 `.runtime`,同步区死水）覆盖部署区 5 文件→PC 权威源恢复→PC 侧脚本也被 NUC 架构取代撤销。**PC `.runtime/tokens/` 已降级为历史快照可清理**（详见 CREDENTIALS.md §NUC 统一凭证提取器） |
