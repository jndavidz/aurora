...
// codingResponses 处理 Gemini coding 变体(/v1/responses)。
//
// 定位:云端能力助手(与智谱/Grok coding 同款,见 docs/GLM.md §四)。
// Gemini 网页版模型有原生搜索/地图等能力,但客户端自定义工具调用不保证。
// 提示词引导模型输出围栏 JSON,FenceParser 捕获(尽力而为)。
...
func (d *Gemini) codingResponsesStream(c *gin.Context, req *official.ResponsesAPIRequest, client *geminweb.Client, streamReq geminweb.CompletionRequest) {
	w := newSSEWriter(c)
...
func (d *Gemini) codingResponsesNonStream(c *gin.Context, req *official.ResponsesAPIRequest, client *geminweb.Client, streamReq geminweb.CompletionRequest) {
	var fullText string
...
// codingCompletions 处理 Gemini coding 变体(/v1/chat/completions)。
...
func (d *Gemini) codingCompletionsStream(c *gin.Context, req *official.APIRequest, client *geminweb.Client, streamReq geminweb.CompletionRequest) {
	model := req.Model
	if model == "" {
		model = "auto"
	}
...
func (d *Gemini) codingCompletionsNonStream(c *gin.Context, req *official.APIRequest, client *geminweb.Client, streamReq geminweb.CompletionRequest) {
	var fullText string
	res := client.Complete(streamReq, func(delta geminweb.Delta) { fullText += delta.Text })
	if res.Err != "" && fullText == "" {
		c.JSON(502, gin.H{"error": res.Err})
